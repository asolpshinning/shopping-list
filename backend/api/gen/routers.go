...
import (
	...
	env "github.com/asolpshinning/shopping-list/backend/config"
...
func NewRouter(routers ...Router) *mux.Router {
	...
	allowedOriginsSlice, _ := env.ALLOWED_ORIGINS()
	allowedOrigins := handlers.AllowedOrigins(allowedOriginsSlice)
	allowedMethods := handlers.AllowedMethods(strings.Split(env.ALLOWED_METHODS, ","))
	allowedHeaders := handlers.AllowedHeaders(strings.Split(env.ALLOWED_HEADERS, ","))
	for _, api := range routers {
		for _, route := range api.Routes() {
			var handler http.Handler
			handler = route.HandlerFunc
			handler = Logger(handler, route.Name)
			handler = handlers.CORS(allowedOrigins, allowedMethods, allowedHeaders)(handler)
